...
async function initialLoad() {
  try {
    ...
    if (jsonData.length > 0) {
      await fetchBreedInfo(jsonData[0].id);
    }
  } catch (error) {
...
//     const favExists = favourites.some(fav => fav.image_id === imgId);

//     if (favExists) {
//       const favId = favourites.find(fav => fav.image_id === imgId).id;
//       await axios.delete(`https://api.thecatapi.com/v1/favourites/${favId}`, {
//         headers: { 'x-api-key': API_KEY }
//       });
//       console.log('Favourite removed:', favId);
//     } else {
